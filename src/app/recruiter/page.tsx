...
function RecruiterDashboard() {
  ...
  return <main className="recruiter-shell">
    <header><Link className="brand" href="/"><span className="brand-mark">F</span><span>Foxy</span><span className="brand-mode">Recruiter</span></Link><div><span className="system-live"><span />Live on Convex</span><button className="text-button" onClick={() => void signOut()}>Sign out</button></div></header>
    ...
    {jobs === undefined ? <div className="dashboard-empty">Loading workspace…</div> : jobs.length === 0 ? <div className="dashboard-empty"><h2>No active roles</h2><p>Create a custom role or use the demo data to start.</p></div> : <section className="job-list">{jobs.map((job) => <article key={job._id} className="job-panel"><header><div><span className="role-type">Open role</span><h2>{job.title}</h2>{job.description && <p className="job-description">{job.description}</p>}<p>{job.competencies.length} competencies · {job.applications.length} applications</p></div><button className="secondary-button" onClick={() => invite(job._id)} disabled={working}>Copy candidate invite</button></header><div className="application-table"><div className="table-head"><span>Candidate</span><span>Status</span><span>Score</span><span>Recommendation</span><span>Proctoring</span></div>{job.applications.length === 0 ? <div className="table-empty">No candidates yet. Copy an invite to start.</div> : job.applications.map((application) => <div className="application-row" key={application._id}><span>{application.candidate?.name ?? "Invite pending"}<small>{application.candidate?.githubUrl ?? "Waiting for candidate"}</small></span><span><i className={`application-status status-${application.status}`} />{application.status.replaceAll("_", " ")}</span><span>{application.result?.overallScore ?? "—"}</span><span>{application.result?.recommendation?.replaceAll("_", " ") ?? "—"}</span><span className="recording-links">{application.recordings.length ? application.recordings.map((recording) => recording.url ? <a key={recording.type} href={recording.url} target="_blank" rel="noreferrer">{recording.type}</a> : null) : "—"}</span></div>)}</div>{copied && job.applications.some((application) => application.inviteToken === copied) && <p className="copy-notice">Invite copied. Open it in a private window to test the candidate flow.</p>}</article>)}</section>}
  </main>;
}
