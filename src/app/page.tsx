...
export default async function MarketingHome({ searchParams }: { searchParams: Promise<{ invite?: string }> }) {
  ...
  return <main className="marketing-page">
    ...
    <section className="mechanism" id="how-it-works"><header><h2>Claim → Evidence → Challenge → Verification</h2><p>Every interview follows one transparent chain. Each step keeps its receipts, so every verdict can be traced back to real work.</p></header><div className="mechanism-steps">
      <TimelineStep index={0} title="Claim" showArrow artifact={<span className="step-chip">“Built scalable Node.js APIs.”</span>}>Read the candidate’s résumé and identify specific technical claims.</TimelineStep>
      <TimelineStep index={1} title="Evidence" showArrow artifact={<span className="step-chip"><i className="chip-dot" />express-api · auth, error handling, 34 tests</span>}>Use Context.dev to inspect GitHub projects, tools, tests, and implementation signals.</TimelineStep>
      <TimelineStep index={2} title="Challenge" showArrow artifact={<span className="step-chip">“Walk me through how your auth middleware handles token expiry.”</span>}>Generate role-specific questions tied to the strongest evidence and the important gaps.</TimelineStep>
      <TimelineStep index={3} title="Verification" artifact={<span className="step-chip chip-verified"><i className="chip-dot" />Backend competency verified</span>}>Grade technical accuracy, depth, and requirement alignment with every reason attached.</TimelineStep>
    </div></section>
